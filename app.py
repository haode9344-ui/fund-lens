...
def enrich_holdings(holdings: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    try:
        payload = json.loads(fetch_text(url, timeout=5))
    except Exception:
        return holdings
...
def related_impact(holdings: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "hasRealtimeHoldings": bool(valid),
    }
...
def fetch_sina_7x24(limit: int = 30) -> list[dict[str, Any]]:
    ...
    try:
        payload = json.loads(fetch_text(url, timeout=6))
...
def score_event(item: dict[str, Any], fund_name: str, holdings: list[dict[str, Any]], impact: dict[str, Any]) -> dict[str, Any] | None:
    ...
    foreign_non_core = ["泰国", "印尼", "越南", "菲律宾", "马来西亚", "土耳其", "巴西", "墨西哥", "南非"]
    macro_tokens = [
        "A股",
        "沪深300",
        "中证500",
    ...
    context = " ".join(
        [
            fund_name or "",
            *[str(item.get("name") or "") for item in impact.get("topIndustries", [])],
            *[str(item.get("name") or "") for item in holdings],
        ]
    )
    if fund_name and fund_name in text:
        strong_matches.append(fund_name)
    for token in macro_tokens:
        if token and token in text:
            medium_matches.append(token)
    ...
    for token in theme_tokens:
        if token in text and token in context:
            strong_matches.append(token)
    for holding in holdings:
        for token in [holding.get("name"), holding.get("code")]:
            if token and token in text:
                strong_matches.append(str(token))
    strong_matches.extend(item.get("matched") or [])
    strong_matches = list(dict.fromkeys([m for m in strong_matches if m]))
    medium_matches = list(dict.fromkeys([m for m in medium_matches if m and m not in strong_matches]))

    positive = sum(1 for word in POSITIVE_WORDS if word in text)
    negative = sum(1 for word in NEGATIVE_WORDS if word in text)
    if not strong_matches and any(token in text for token in foreign_non_core):
        return None
    if not strong_matches and not medium_matches:
        return None

    relevance = "强相关" if strong_matches else "中相关"
    matched = strong_matches + medium_matches
    sentiment = "中性"
    if positive > negative:
        sentiment = "正面"
    elif negative > positive:
        sentiment = "负面"
    base = 55 if relevance == "强相关" else 35
    severity = min(100, base + len(strong_matches) * 12 + len(medium_matches) * 6 + max(positive, negative) * 18)
    action = "中等相关，作为市场背景观察，需行情确认"
    if relevance == "强相关" and sentiment == "正面" and severity >= 55:
        action = "可能利好，关注明天是否高开或放量"
    elif relevance == "强相关" and sentiment == "负面" and severity >= 55:
        action = "可能利空，谨慎加仓并观察重仓股承压"
    elif relevance == "强相关":
        action = "强相关但方向未明，等待重仓股和行业行情验证"
...
def load_monitor(code: str) -> dict[str, Any]:
    ...
    return {
        ...
        "alerts": alerts[:12],
        "note": "本地监控会抓取新浪7x24快讯和东方财富重仓股公告，并按强相关/中相关过滤；弱相关宏观新闻不进入预警。新闻利好/利空与实际涨跌可能偏离，需结合行情确认。",
    }
